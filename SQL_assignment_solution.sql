...
select Region, count(*) as no_of_customers from customer 
group by Region order by no_of_customers 
DESC limit 1;

# Another solution that does not use LIMIT clause

select Region, count(*) as no_of_customers 
from customer
group by Region
having no_of_customers = (select max(no_of_customers) from (
	select Region, count(*) as no_of_customers
	from customer 
	group by Region
	) x
...
#Find the number and id of product sold in decreasing order of product sold (display product id, no_of_product sold) 
select Prod_id, count(*) as no_of_product_sold
      from market_fact
      group by Prod_id
...
select Customer_Name, sum(Order_Quantity) as num_tables
      from market_fact s, customer c, product p
